...
def eval_speed(first_local_maximum_ted, first_local_maximum_you, df, df1):
    if first_local_maximum_you > first_local_maximum_ted:  ## A음성(first1)이 B음성(first)보다 음성이 늦게 시작한다면
        ...
        if len(blue_ted_added) > len(df1[0]):
            index_diff = abs((len(blue_ted_added) - (len(df1[0]))))
            time_diff = index_diff / 16000
            time_diff_per = index_diff / (len(blue_ted_added) - first_local_maximum_you)
            time_result = round((1 - time_diff_per) * 100, 1)

        else:
            index_diff = abs((len(df1[0])) - (len(blue_ted_added)))
            time_diff = index_diff / 16000
            time_diff_per = index_diff / (len(blue_ted_added) - first_local_maximum_you)
            time_result = round((1 + time_diff_per) * 100, 1)
    ...
    # 테듣가 더길떄               ######## 방금 위에한거의 반대과정 A보다 B가 늦게시작할경우######
        ...
        if len(red_you_added) > len(df[0]):
            index_diff = abs((len(red_you_added) - (len(df[0]))))
            time_diff = index_diff / 16000
            time_diff_per = index_diff / (len(df[0]) - first_local_maximum_ted)
            time_result = round((1 + time_diff_per) * 100, 1)
        else:
            index_diff = abs((len(df[0])) - (len(red_you_added)))
            time_diff = index_diff / 16000
            time_diff_per = index_diff / (len(df[0]) - first_local_maximum_ted)
            time_result = round((1 - time_diff_per) * 100, 1)
...
def eval_strength(first_local_maximum_ted, first_local_maximum_you, df, df1, png_save_path):
    ...
    if first_local_maximum_you > first_local_maximum_ted:  ## A음성(first1)이 B음성(first)보다 음성이 늦게 시작한다면
        ...
        if len(blue_ted_added) > len(df1[0]):
            ...
            for z in range(first_local_maximum_you, len(df1[0]), 1):
                area.append(blue_ted_added[z])
            # points = 두 파형의 넓이 비교를 통한 점수계산
            points = 1 - (sum(abs(df1[0][first_local_maximum_you:len(df1[0])] - blue_ted_added[
                                                                                first_local_maximum_you:len(
                                                                                    df1[0])])) / sum(area))
            # points = 코사인 유사도
            points3 = cos_sim(blue_ted_added[first_local_maximum_you:len(df1[0])],
            ...
            ranks1 = []
            for i in range(1, len(blue_ted_added) - 1, 1):
                if blue_ted_added[i] > 0.15:
                    if blue_ted_added[i] > blue_ted_added[i - 1] and blue_ted_added[i] > blue_ted_added[i + 1]:
                        ranks1.append(blue_ted_added[i] / max(blue_ted_added))
            diffrent = []
            ...
            area = []
            for z in range(first_local_maximum_you, len(blue_ted_added), 1):
                area.append(blue_ted_added[z])
            points = 1 - (sum(abs(df1[0][first_local_maximum_you:len(blue_ted_added)] - blue_ted_added[
                                                                                         first_local_maximum_you:len(
                                                                                             blue_ted_added)])) / sum(
                area))
            points3 = cos_sim(blue_ted_added[first_local_maximum_you:len(blue_ted_added)],
                              df1[0][first_local_maximum_you:len(blue_ted_added)])
    ...
    line2, = plt.plot(red_graph, color='plum', linewidth=5)
...
def eval_pitch(ted_audio_path, user_audio_path, png_save_path):
    ...
    line2, = plt.plot(df_red, color='plum', linewidth=5)
